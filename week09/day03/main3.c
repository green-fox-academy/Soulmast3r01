...
int main(void) {
	...
	while (1) {
		...
		while (1) {
			char c;
			HAL_UART_Receive(&uart_handle, &c, 1, 0xFFFFFFFF);
			if (c == '\n')
				break;

			counter++;
			input_str = realloc(input_str, counter);
			input_str[counter - 1] = c;
		}

		counter++;
		input_str = realloc(input_str, counter);
		input_str[counter - 1] = '\0';

		printf("%s\r\n", input_str);

		if(strcmp("green",input_str)== 0){
